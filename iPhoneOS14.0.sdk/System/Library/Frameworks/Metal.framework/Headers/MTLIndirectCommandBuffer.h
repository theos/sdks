...
typedef NS_OPTIONS(NSUInteger, MTLIndirectCommandType) {
    ...
    MTLIndirectCommandTypeConcurrentDispatch  API_AVAILABLE(ios(13.0),macos(10.16)) = (1 << 5), /* Dispatch threadgroups with concurrent execution */

     MTLIndirectCommandTypeConcurrentDispatchThreads  API_AVAILABLE(ios(13.0),macos(10.16)) = (1 << 6), /* Dispatch threads with concurrent execution */
 ...
 */
@property (readwrite, nonatomic) NSUInteger maxKernelBufferBindCount API_AVAILABLE(ios(13.0),macos(10.16));
...
- (id <MTLIndirectComputeCommand>)indirectComputeCommandAtIndex:(NSUInteger)commandIndex API_AVAILABLE(ios(13.0),macos(10.16));
