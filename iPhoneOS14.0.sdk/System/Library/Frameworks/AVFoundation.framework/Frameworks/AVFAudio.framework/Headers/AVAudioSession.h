...
/*!
	...
	remote I/Os, etc.). Starting in iOS 8, if the session has running IOs at the time that
	...
	misuse of the API. Prior to iOS 8, the session would have remained active if it had running IOs
...
/*!
	...
	This is relatively time consuming operation. The completion handler will be called when the
...
/* Sets the preferred input orientation. The input orientation determines which directions will be left and right
...
/*!
    ...
    again.
*/
OS_EXPORT NSString *const AVAudioSessionInterruptionWasSuspendedKey API_AVAILABLE(ios(10.3), watchos(3.2), tvos(10.3));
