...
/*! @abstract Starts app recording with a completion handler. Note that before recording actually starts, the user may be prompted with UI to confirm recording.
 @discussion handler Called after user interactions are complete. Will be passed an optional NSError in the RPRecordingErrorDomain domain if there was an issue starting the recording.
 */
- (void)startRecordingWithHandler:(nullable void (^)(NSError *_Nullable error))handler API_AVAILABLE(ios(10.0), tvos(10.0), macCatalyst(10.15), macos(10.16));
 ...
 */
- (void)stopRecordingWithOutputURL:(NSURL *)url completionHandler:(nullable void (^)(NSError *_Nullable error))completionHandler NS_SWIFT_NAME(stopRecording(withOutput:completionHandler:)) API_AVAILABLE(ios(14.0), tvos(14.0), macCatalyst(10.16), macos(10.16));
 ...
 */
- (void)startCaptureWithHandler:(nullable void (^)(CMSampleBufferRef sampleBuffer, RPSampleBufferType bufferType, NSError *_Nullable error))captureHandler completionHandler:(nullable void (^)(NSError *_Nullable error))completionHandler API_AVAILABLE(ios(11.0), tvos(11.0), macCatalyst(10.15), macos(10.16));
 ...
 */
- (void)stopCaptureWithHandler:(nullable void (^)(NSError *_Nullable error))handler API_AVAILABLE(ios(11.0), tvos(11.0), macCatalyst(10.15), macos(10.16));
...
@property (nonatomic, getter = isCameraEnabled) BOOL cameraEnabled API_AVAILABLE(ios(10.0), macCatalyst(10.15), macos(10.16)) API_UNAVAILABLE(tvos);

/* @abstract Specify or query camera position. Can be used for key value observing. Default is RPCameraPositionFront. */
@property (nonatomic) RPCameraPosition cameraPosition API_AVAILABLE(ios(11.0), macCatalyst(10.15), macos(10.16)) API_UNAVAILABLE(tvos);
...
#else
@property (nonatomic, readonly, nullable) NSView *cameraPreviewView API_AVAILABLE(ios(10.0), macCatalyst(10.15), macos(10.16)) API_UNAVAILABLE(tvos);
#endif
@end

API_AVAILABLE(ios(9.0), tvos(10.0), macCatalyst(10.15), macos(10.16))
 ...
 */
- (void)screenRecorder:(RPScreenRecorder *)screenRecorder didStopRecordingWithError:(NSError *)error previewViewController:(nullable RPPreviewViewController *)previewViewController API_DEPRECATED("No longer supported", ios(9.0, 10.0), tvos(10.0, 10.0), macCatalyst(10.15, 10.15)) API_UNAVAILABLE(macos);
- (void)screenRecorder:(RPScreenRecorder *)screenRecorder didStopRecordingWithPreviewViewController:(nullable RPPreviewViewController *)previewViewController error:(nullable NSError *)error API_AVAILABLE(ios(11.0), tvos(11.0), macCatalyst(10.15), macos(10.16));
