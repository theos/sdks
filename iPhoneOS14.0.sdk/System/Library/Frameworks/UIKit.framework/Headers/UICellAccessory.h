...
/// filled circle with checkmark when selected). The inherited tintColor property affects the deselected state.
UIKIT_EXTERN API_AVAILABLE(ios(14.0), tvos(14.0), watchos(7.0))
@interface UICellAccessoryMultiselect : UICellAccessory

/// The tint color to apply to the selected state (filled circle with checkmark). Default is nil, which uses system default.
...
/// The tint color to apply when the containing cell is selected. Default is nil, which uses system default.
@property (nonatomic, strong, nullable) UIColor *selectedTintColor;
